...
export function parseArgs(args: string[]): Options {
    ...
    for (let i = 0; i < args.length; i++) {
        switch (args[i]) {
            case "-s":
            case "--server":
                if (args[i + 1] && CONFIGS[args[i + 1] as keyof typeof CONFIGS]) {
                    options.server = args[i + 1] as keyof typeof CONFIGS;
                    i++;
                } else {
                    throw new Error(
                        `Invalid or missing value for "--server". Supported servers: ${Object.keys(CONFIGS).join(", ")}.`
                    );
                }
                break;
            case "-d":
...
export async function validateAndPrepareConfig(options: Options) {
    const { server, dir } = options;

    // Check if the server is supported
    if (!CONFIGS[server]) {
        throw new Error(
            `Unsupported server "${server}". Supported servers: ${Object.keys(CONFIGS).join(", ")}.`
        );
    }
